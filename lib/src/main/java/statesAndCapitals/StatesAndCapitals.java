...
public class StatesAndCapitals
{
    ...
    public void testStatesAndCapitals() throws IOException
    {
        ...
        StateInfo cardinalState = states.stream().filter(s -> s.getStateBird().equals("cardinal")).findAny().orElseThrow(null);;
        ...
        List<String> allDistinctStateBirdsMinusMockingbirds =  states.stream().filter(s -> !s.getStateBird().contains("mockingbird")).map(StateInfo::getStateBird).distinct().collect(toList());
